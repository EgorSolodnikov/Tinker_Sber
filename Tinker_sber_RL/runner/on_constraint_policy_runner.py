...
class OnConstraintPolicyRunner:
    ...
    def save(self, path, infos=None):
        model_info = {
        'class_name': self.cfg["policy_class_name"],
        'module_path': self.alg.actor_critic.__module__,
        'class_def': str(self.alg.actor_critic.__class__),
        'init_params': {
            'n_proprio': self.env.cfg.env.n_proprio,
            'n_scan': self.env.cfg.env.n_scan,
            'num_obs': self.env.num_obs,
            'n_priv_latent': self.env.cfg.env.n_priv_latent,
            'history_len': self.env.cfg.env.history_len,
            'num_actions': self.env.num_actions,
            'policy_cfg': self.policy_cfg
        },
        'state_dict_shapes': {name: param.shape 
                              for name, param in self.alg.actor_critic.state_dict().items()},
        'model_architecture': str(self.alg.actor_critic)
    }
    ...
    # Дополнительно: сохраняем текстовый файл с описанием модели
        with open(path.replace('.pt', '_arch.txt'), 'w') as f:
            f.write("Model Architecture:\n")
            f.write(str(self.alg.actor_critic))
            f.write("\n\nState Dict Shapes:\n")
            for name, shape in state_dict['model_info']['state_dict_shapes'].items():
